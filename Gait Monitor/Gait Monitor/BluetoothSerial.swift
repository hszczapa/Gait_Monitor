...
final class Bluetooth: BluetoothSerialDelegate {
    
    init() {
        serial = BluetoothSerial(delegate: self)
        //serial.connectToPeripheral(<#T##peripheral: CBPeripheral##CBPeripheral#>)
    }
    
    func serialDidChangeState() {
        // DO something else when we change state
    }
    
    func serialDidDisconnect(_ peripheral: CBPeripheral, error: NSError?) {
        //Do something when we disconnect
    }
...
final class BluetoothSerial: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    var serviceUUID = CBUUID.init(string: "FFE0")
    
    /// UUID of the characteristic to look for.
    var characteristicUUID = CBUUID.init(string: "FFE1")
